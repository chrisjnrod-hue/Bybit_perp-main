...
async def start_background_tasks(app: web.Application):
    ...
    # Small deterministic stdout marker can help Render pick up startup
    print("[SURGICAL_START] scanner task created and background scanner.run() started", flush=True)
...
async def debug_symbol(request: web.Request):
    ...
    # normalize some common aliases so callers can request 1h/4h/1d etc.
    tf_alias_map = {
        "1h": "60", "1H": "60", "60": "60",
    ...
    flip = False
    try:
        # compute small dynamic threshold locally for debug, same logic scanner uses in logs
        try:
            hist_std = statistics.pstdev(hist_list) if len(hist_list) >= 2 else 0.0
        except Exception:
            hist_std = 0.0
        hist_threshold = max(1e-9, abs(hist_std) * 0.05)
        flip = bool(scanner.detect_flip_current_open(hist_list, hist_threshold, symbol=symbol, tf=tf))
    except Exception:
        flip = False
    ...
    try:
        ...
        if last_candles:
            ...
            # normalize milliseconds->seconds when needed for human-friendly age
            if start_at and start_at > 10000000000:
