    ...
    def detect_flip_current_open(self, hist: List[float], hist_threshold: float = 0.0, symbol: str = "", tf: str = ""):
        ...
        try:
            ...
            if result and DEBUG_SURGICAL_LOGS:
                logger.warning("[FLIP_DETECTED_INTERNAL] %s %s: STRONG FLIP! prev=%.8f â†’ cur=%.8f (change=%.8f)", 
        ...
        while not self._stop:
            ...
            try:
                ...
                try:
                    ...
                    logger.info("âœ“ ROOT_SCAN_COMPLETE: checked=%d, signals=%d, candidates=%d", 
        ...
        for rt in ROOT_TFS:
            ...
            for s, p, v in lst:
                if v is None:
                    ...
                    lines.append(f"- {s} @ {p} (24h vol Î” {v:.2f})")
