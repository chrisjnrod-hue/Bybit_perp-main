...
def compute_macd_from_closes(closes: List[float], include_price: Optional[float] = None, fast: int = 12, slow: int = 26, signal: int = 9):
    ...
    if len(data) < min_window:
        print(f"[MACD_UNDERFLOW] CRITICAL: {len(data)} closes < minimum {min_window} for MACD calculation")
        print(f"[MACD_UNDERFLOW] This means klines were NOT seeded properly or are empty")
        return None, None, []
