...
# Manages symbol discovery, kline seeding, REST polling, and root scan loop coordination
...
from .config import (
    ...
    ROOT_TFS, MTF_TFS, ROOT_SCAN_INTERVAL, USE_WS,
...
class ScannerOrchestrator:
    """Manages symbol discovery, kline seeding, and REST polling."""
    
    def __init__(self, client: BybitClient, trade_manager: TradeManager, rate_limiter: TokenBucket):
        self.client = client
        ...
        logger.info(
            "orchestrator initialized (USE_WS=%s SEED_KLINES_LIMIT=%d CONCURRENCY=%d DEBUG_SURGICAL=%s DIAGNOSTIC=%s)",
    ...
    async def seed_klines_for_symbol(self, symbol: str):
        ...
        tfs = list(set(ROOT_TFS + MTF_TFS + ["5", "15", "60", "240", "D"]))
        for tf in tfs:
            try:
                ...
                if DEBUG_SURGICAL_LOGS:
                    self._log_raw_response(symbol, tf, raw)
    ...
    def _log_raw_response(self, symbol: str, tf: str, raw: Any):
    ...
    async def _update_24h_volume(self, symbol: str) -> Optional[float]:
        """Update and track 24h volume data."""
        try:
